...
export function selectCodexMainTargets(targets: DebugTarget[]): DebugTarget[] {
  ...
  return [...candidates].sort((left, right) => {
    const score = (target: DebugTarget): number => {
      if (isIndexDocument(target) && !new URL(target.url).search) return 0;
      if (isIndexDocument(target) && !isAuxiliarySurface(target)) return 1;
      if (!isAuxiliarySurface(target) && !target.url.includes("initialRoute=")) return 2;
      if (!isAuxiliarySurface(target)) return 3;
      return 4;
    };
...
export function selectCodexMainTarget(targets: DebugTarget[]): DebugTarget | undefined {
  return selectCodexMainTargets(targets)[0];
}

export function isRetryableSnapshotInitializationError(error: unknown): boolean {
  const message = error instanceof Error ? error.message : String(error);
  return /Codex (?:application module is not loaded|Micro slot signals are not loaded|Micro settings definitions were not found|VS Code event bus was not found|React root was not found|Micro slot resolver was not found|Micro slot store was not found)/.test(message);
}

export function normalizedKeycapError(keycapId: OfficialKeycapId, error: unknown): Error {
  const raw = error instanceof Error ? error.message : String(error);
  const message = raw.replace(/^Error:\s*/i, "");
  if (/not active in the current view|no action|not supported/.test(message)) {
    return new Error(`${keycapId}: The current Codex view cannot execute this keycap.`);
  }
  return new Error(`${keycapId}: ${message}`);
}

function isKeycapTransportError(error: unknown): boolean {
  const message = error instanceof Error ? error.message : String(error);
  return /Codex-(?:Micro-)?Brücke|Codex-Runtime-Antwort|CDP-Fehler|WebSocket|socket/i.test(message);
}
...
const SNAPSHOT_EXPRESSION = `(async () => {
  ...
  if (!appInitialUrl) throw new Error('Codex application module is not loaded.');
  if (!slotSignalsUrl) throw new Error('Codex Micro slot signals are not loaded.');

  // app-initial is already the running renderer entry module. Importing every
  // observed lazy asset executes unrelated route modules and can trip the
  // Codex error boundary. Snapshot collection must remain read-only.
  const namespaces = [await import(appInitialUrl), await import(slotSignalsUrl)];
  ...
  const slots = found.slots.map((slot) => ({
    ...slot,
    projectName: (() => {
      const direct = slot.projectName ?? slot.project?.name ?? slot.workspace?.name ?? slot.repository?.name;
      if (typeof direct === 'string' && direct.trim()) return direct.trim().slice(0, 120);
      const path = slot.cwd ?? slot.workingDirectory ?? slot.project?.path ?? slot.workspace?.path;
      if (typeof path !== 'string' || !path.trim()) return null;
      return path.replace(/[\\/]+$/, '').split(/[\\/]/).pop()?.slice(0, 120) ?? null;
    })(),
  ...
  // Resolve only the known Codex Micro layout module. Never execute unrelated
  // lazy route modules while collecting a snapshot.
  const officialKeycapIds = ${JSON.stringify(OFFICIAL_KEYCAP_IDS)};
  const keycapRegistryUrl = urls.find((url) => url.includes('/assets/codex-micro-layout-'));
  const keycapCapabilities = [];
  if (keycapRegistryUrl) {
    const keycapRegistry = await import(keycapRegistryUrl);
    const keycapDefinitions = Object.values(keycapRegistry).find((candidate) =>
      Array.isArray(candidate) && candidate.length >= officialKeycapIds.length &&
      officialKeycapIds.every((id) => candidate.some((item) => item?.id === id))
    );
    if (Array.isArray(keycapDefinitions)) {
      for (const id of officialKeycapIds) {
        const definition = keycapDefinitions.find((item) => item?.id === id);
        const action = definition?.action;
        const actionType = action?.type === 'named' && action?.label === 'Push to talk'
          ? 'push-to-talk'
          : ['command', 'external-url', 'composer-text'].includes(action?.type)
            ? action.type
            : null;
        if (actionType) keycapCapabilities.push({
          id, actionType, status: 'ready', danger: false
        });
      }
    }
  }
  const availableKeycaps = keycapCapabilities.filter((item) => item.status === 'ready').map((item) => item.id);
  const approvalPending = document.querySelector('[data-codex-approval-surface]') != null;
  ...
  return { slots, activeThreadKey, activeThreadTitle, layout, agentSource, lightingAutoOff, theme,
    availableKeycaps, keycapCapabilities, approvalPending, ...(usage ? { usage } : {}) };
})()`;
...
export class CodexMicroRendererBridge {
  ...
  async refresh(): Promise<MicroSnapshot> {
    ...
    for (let attempt = 0; attempt < 4; attempt += 1) {
      try {
        ...
        this.preferredTargetId = this.currentTargetId;
        this.targetCursor = 0;
        this.lastSnapshot = snapshot;
        return snapshot;
      } catch (error) {
        lastError = error;
        const failedTarget = this.currentTargetId;
        this.disconnect();
        if (!isRetryableSnapshotInitializationError(error) || attempt === 3) throw error;
        if (failedTarget && this.preferredTargetId === failedTarget) this.preferredTargetId = undefined;
        this.targetCursor += 1;
        this.log(`Codex Micro snapshot target was not ready; trying another renderer (${attempt + 2}/4).`);
  ...
  async tapAgent(slot: number, expectedThreadKey: string): Promise<void> {
    if (!Number.isInteger(slot) || slot < 0 || slot > 5) throw new Error(`Ungültiger Micro-Agent-Slot: ${slot}`);
    const snapshot = await this.refresh();
    const plan = resolveAgentDispatch(snapshot, slot, expectedThreadKey);
    if (plan.kind === "native") {
      if (plan.slot !== slot) this.log(`Agent slot ${slot + 1} changed before tap; using native slot ${plan.slot + 1}.`);
      const event = (act: 0 | 1) => this.dispatch("codex-micro-hid-event", {
        event: { key: `AG0${plan.slot}`, act, slot: plan.slot, threadKey: plan.threadKey }
      }, "codex-micro-hid-event");
      await event(1);
  ...
  async runKeycap(keycapId: OfficialKeycapId, act?: 0 | 1): Promise<void> {
    if (!OFFICIAL_KEYCAP_IDS.includes(keycapId)) throw new Error(`Unknown Codex Micro keycap: ${keycapId}`);
    if (keycapId === "MIC" && act == null) throw new Error("MIC requires an explicit press or release state.");
    if (keycapId !== "MIC" && act != null) throw new Error(`Keycap ${keycapId} does not accept a press state.`);
    if (keycapId === "MIC") {
      const snapshot = await this.refresh();
      const assigned = Object.entries(snapshot.layout.slots).find(([, value]) => value?.keycapId === "MIC")?.[0];
      if (assigned && ["ACT06", "ACT07", "ACT08", "ACT09", "ACT10_ACT11", "ACT12"].includes(assigned)) {
        await this.sendAction(assigned as MicroActionSlot, act!);
        return;
      }
    }
    const beforeThreadKey = keycapId === "SPLIT" ? (await this.refresh()).activeThreadKey : undefined;
    await this.ensureConnected();
    const expression = `(async () => {
      ...
      const keycapDefinitions = Object.values(layout).find((candidate) =>
        Array.isArray(candidate) && candidate.some((item) => item?.id === 'FAST') && candidate.some((item) => item?.id === ${JSON.stringify(keycapId)})
      );
      if (!Array.isArray(keycapDefinitions)) throw new Error('Codex Micro keycap registry changed.');
      const keycap = keycapDefinitions.find((item) => item?.id === ${JSON.stringify(keycapId)});
      const action = keycap?.action;
      if (!action) throw new Error('The selected Codex Micro keycap has no action.');

      if (action.type === 'named' && action.label === 'Push to talk') {
        const stateKey = Symbol.for('codex-micro-mobile-push-to-talk');
        const labels = /^(音声入力|Voice input|Dictation)$/i;
        const button = [...document.querySelectorAll('button')].find((element) => labels.test(element.getAttribute('aria-label') || ''));
        if (!button || typeof button.click !== 'function') throw new Error('Codex Push to talk control is unavailable in the current view.');
        const active = Boolean(globalThis[stateKey]);
        if (${act === 1 ? "true" : "false"} && !active) { button.click(); globalThis[stateKey] = true; }
        if (${act === 0 ? "true" : "false"} && active) { button.click(); globalThis[stateKey] = false; }
        return true;
      }
      ...
      if (action.type === 'external-url' && !vscodeUrl) {
    ...
    try {
      ...
      if (keycapId === "SPLIT" && beforeThreadKey && isKeycapTransportError(error)) {
        this.disconnect();
        ...
        while (Date.now() < deadline) {
          try {
            ...
            if (current.activeThreadKey && current.activeThreadKey !== beforeThreadKey) return;
          } catch { /* renderer navigation is still settling */ }
        }
      }
      if (isKeycapTransportError(error)) this.disconnect();
      throw normalizedKeycapError(keycapId, error);
    }
  }
  ...
  async consumeRateLimitReset(): Promise<void> {
    ...
    const expression = `(async () => {
      ...
      const appInitialUrl = urls.find((url) => url.includes('/assets/app-initial-'));
      if (appInitialUrl) {
        const namespace = await import(appInitialUrl);
  ...
  private async connect(): Promise<void> {
    ...
    const candidates = selectCodexMainTargets(targets).filter((target) =>
      !/avatar-overlay|composition-surface/i.test(target.url)
    );
    const ordered = this.preferredTargetId
      ? [...candidates].sort((left, right) => Number(targetIdentity(right) === this.preferredTargetId) - Number(targetIdentity(left) === this.preferredTargetId))
      : candidates;
    const target = ordered[this.targetCursor % Math.max(ordered.length, 1)];
    ...
    this.socket = socket;
    this.currentTargetId = targetIdentity(target);
...
function targetIdentity(target: DebugTarget): string {
  return target.id ?? target.webSocketDebuggerUrl ?? target.url;
}
