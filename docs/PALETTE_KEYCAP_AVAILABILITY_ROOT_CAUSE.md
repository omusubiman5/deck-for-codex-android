...
| 1.1 | 2026-08-20 | 廃止 | 27キー全件試験で判明したlive smoke NIC誤選択、OAI fallback、CDP切断を追補 |
| 1.2 | 2026-08-20 | 現行 | APPR／REJ／DELの危険分類が過剰設計だったことを追補し、30キー統合を正本化 |

> 6 Action slotだけをavailabilityとした直接原因は有効である。一方、本書後半の通常27／Danger 3という是正案は後続レビューで撤回した。現行仕様は`PALETTE_30_KEY_RESTORE_IMPLEMENTATION_PLAN.md`文書版1.0を正とする。
