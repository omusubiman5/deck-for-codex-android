...
type RelayControl = Pick<CodexMicroRendererBridge,
  "refresh" | "tapAgent" | "sendAgent" | "sendAction" | "sendJoystick" | "sendEncoder" | "adjustReasoning" |
  "runKeycap" | "runEnvironmentAction" | "consumeRateLimitReset">;
...
export class CodexRelayServer {
  ...
  private agentTapInFlight = false;
  private readonly micActive = new Set<WebSocket>();
  ...
  private async closeBound(): Promise<void> {
    ...
    for (const socket of server?.clients ?? []) { this.releaseConnectionState(socket); socket.terminate(); }
    this.authenticated.clear();
    this.micActive.clear();
  ...
  private handleConnection(socket: WebSocket): void {
    ...
    socket.once("message", (raw) => {
      ...
      socket.on("close", () => this.releaseConnectionState(socket));
      socket.on("error", () => this.releaseConnectionState(socket));
  ...
  private async handleMessage(socket: WebSocket, raw: string): Promise<void> {
    ...
    let ownsAgentTap = false;
    const commandLabel = command.kind === "agent"
      ? `agent:${command.slot + 1}:${command.act === 1 ? "down" : "up"}`
      : command.kind === "agent-tap" ? `agent:${command.slot + 1}:tap`
      : command.kind;
    this.log(`Relay command ${commandLabel} received.`);
    try {
      if (command.kind === "agent-tap" && this.agentTapInFlight) {
        throw new Error("Another agent selection is already in progress.");
      }
      if (command.kind === "agent-tap") { this.agentTapInFlight = true; ownsAgentTap = true; }
      if (command.kind === "host-target") {
        if (command.hostId !== this.host.hostId) throw new Error("The requested host is not this relay host.");
      } else await executeRelayCommand(this.control, command);
      if (command.kind === "keycap" && command.keycapId === "MIC") {
        if (command.act === 1) this.micActive.add(socket); else this.micActive.delete(socket);
      }
      this.sendResult(socket, message.requestId, true);
      this.log(`Relay command ${commandLabel} completed in ${Date.now() - startedAt} ms.`);
      void this.publishSnapshot().catch((error) => this.handleSnapshotFailure(error));
    } catch (error) {
      ...
      if (ownsAgentTap) this.agentTapInFlight = false;
    }
  }

  private sendResult(socket: WebSocket, requestId: string, ok: boolean, error?: string, data?: Record<string, unknown>): void {
    if (socket.readyState !== WebSocket.OPEN) return;
    const result: RelayResultMessage = { type: "result", protocol: RELAY_PROTOCOL_VERSION, requestId, ok,
      ...(error ? { error } : {}), ...(data ? { data } : {}) };
    socket.send(JSON.stringify(result));
  }

  private releaseConnectionState(socket: WebSocket): void {
    this.authenticated.delete(socket);
    if (this.micActive.delete(socket)) {
      void this.control.runKeycap("MIC", 0).catch((error) => this.log(`Push to talk release after disconnect failed: ${String(error)}`));
    }
  }
...
export function validateRelayServerConfig(config: RelayServerConfig): void {
  ...
  if (!host || !localHost) {
    throw new Error("Codex Micro Relay accepts only secure auto/private-LAN local mode.");
  }
...
async function executeRelayCommand(control: RelayControl, command: RelayCommand): Promise<void> {
  if (command.kind === "agent-tap") return control.tapAgent(command.slot, command.threadKey);
  ...
  if (command.kind === "new-task") return control.runKeycap("NEW");
  if (command.kind === "environment-action") return control.runEnvironmentAction(command.slot);
  if (command.kind === "host-target") return;
  return control.runKeycap(command.keycapId as OfficialKeycapId, command.kind === "keycap" ? command.act : undefined);
}
