...
export type RelayCommand =
  | { kind: "agent-tap"; slot: number; threadKey: string }
  ...
  | { kind: "keycap"; keycapId: OfficialKeycapId; act?: 0 | 1 }
  | { kind: "new-task" }
  | { kind: "environment-action"; slot: 1 | 2 | 3 }
  | { kind: "host-target"; hostId: string };

export type RelayAuthMessage = { type: "auth"; protocol: 2; token: string };
export const RELAY_CAPABILITIES = [
  "agent-tap", "agent", "action", "joystick", "encoder", "reasoning", "keycap", "new-task",
  "environment-action", "host-target", "usage", "rate-limit-reset"
] as const;
...
export type RelayCommandMessage = { type: "command"; protocol: 2; requestId: string; command: RelayCommand };
export type RelayResultMessage = { type: "result"; protocol: 2; requestId: string; ok: boolean; error?: string; data?: Record<string, unknown> };
...
export function parseRelayCommand(value: unknown): RelayCommand | null {
  ...
  if (value.kind === "agent-tap" && integerIn(value.slot, 0, 5) && isThreadKey(value.threadKey)) return value as RelayCommand;
  ...
  if (value.kind === "keycap" && typeof value.keycapId === "string" && OFFICIAL_KEYCAP_IDS.includes(value.keycapId as OfficialKeycapId)) {
    if (value.keycapId === "MIC") return binary(value.act) ? value as RelayCommand : null;
    if (value.act != null) return null;
    if (value.confirmationNonce != null || value.confirmedHoldMs != null) return null;
    return value as RelayCommand;
  }
  if (value.kind === "new-task") return value as RelayCommand;
  if (value.kind === "environment-action" && integerIn(value.slot, 1, 3)) return value as RelayCommand;
  if (value.kind === "host-target" && typeof value.hostId === "string" && /^[A-Za-z0-9._:-]{1,128}$/.test(value.hostId)) return value as RelayCommand;
  return null;
}
...
function isSnapshot(value: unknown): value is MicroSnapshot {
  ...
  if (!value.slots.every((slot, index) => isRecord(slot) && slot.id === index && typeof slot.status === "string" &&
    (slot.projectName == null || (typeof slot.projectName === "string" && slot.projectName.length <= 120)) &&
  ...
  if (value.availableKeycaps != null && (!Array.isArray(value.availableKeycaps) ||
    value.availableKeycaps.length > OFFICIAL_KEYCAP_IDS.length ||
    !value.availableKeycaps.every((id) => typeof id === "string" && OFFICIAL_KEYCAP_IDS.includes(id as OfficialKeycapId)))) return false;
  if (value.keycapCapabilities != null && (!Array.isArray(value.keycapCapabilities) ||
    value.keycapCapabilities.length > OFFICIAL_KEYCAP_IDS.length ||
    !value.keycapCapabilities.every((item) => isRecord(item) && typeof item.id === "string" &&
      OFFICIAL_KEYCAP_IDS.includes(item.id as OfficialKeycapId) &&
      ["command", "external-url", "composer-text", "push-to-talk"].includes(String(item.actionType)) &&
      ["ready", "unsupported"].includes(String(item.status)) && typeof item.danger === "boolean") ||
    new Set(value.keycapCapabilities.map((item) => (item as Record<string, unknown>).id)).size !== value.keycapCapabilities.length)) return false;
  if (value.approvalPending != null && typeof value.approvalPending !== "boolean") return false;
